...
import type { AtelierCategoryStat, CategoryAtelierGridProps } from "@/features/atelier/types";
...
export default function CategoryAtelierGrid({ categories, currency, language }: CategoryAtelierGridProps) {
  ...
  const customOrderCategories = useMemo(
    () => normalizedCategoryIds.map((id) => categoryById.get(id)).filter((category): category is AtelierCategoryStat => Boolean(category)),
