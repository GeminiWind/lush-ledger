...
import type { EditSavingsPlanDialogProps } from "@/features/savings/types";
...
export default function EditSavingsPlanDialog({ plan, trigger = "card" }: EditSavingsPlanDialogProps) {
  ...
  const editPlanMutation = useMutation({
    ...
    onSuccess: async () => {
      setOpen(false);
      toast.success(t("savings.savingsPlanEditSuccess"));
  ...
  const formik = useFormik({
    ...
    validate: (values) => {
      ...
      if (!Number.isFinite(targetAmount) || targetAmount <= 0) {
        ...
        errors.targetAmount = t("savings.savingsPlanTargetMinSaved").replace("{amount}", formatCurrency(plan.saved, currency));
      }
    ...
    <>
      {trigger === "primary" ? (
        <button
          ...
          title={t("savings.savingsPlanEditAction")}
          aria-label={t("savings.savingsPlanEditAction")}
        >
        ...
        >
          ...
          <span>{t("savings.savingsPlanEditAction")}</span>
        </button>
      )}
          ...
          >
            <div className="flex items-center justify-between gap-4 border-b border-[#d4ecf9] bg-white/70 px-8 py-6 backdrop-blur-sm md:px-10 md:py-8">
              <div>
                <h2 className="font-[var(--font-manrope)] text-2xl font-extrabold tracking-tight text-[#1b3641] md:text-3xl">
                  {t("savings.savingsPlanEditTitle")}
                </h2>
                <p className="mt-1 max-w-2xl text-sm font-medium text-[#49636f]">
                  {t("savings.savingsPlanEditSubtitle")}
                </p>
              </div>
            ...
            <form onSubmit={formik.handleSubmit}>
                  ...
                  <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                    <div className="space-y-2">
                      ...
                      <p className="text-[11px] text-[#647e8c]">
                        {t("savings.savingsPlanCurrentSavedHint").replace("{amount}", formatCurrency(plan.saved, currency))}
                      </p>
              ...
              <div className="flex items-center justify-end gap-4 border-t border-[#d4ecf9] bg-white px-8 py-6 md:px-10 md:py-8">
                ...
                >
                  <span>{editPlanMutation.isPending ? t("savings.savingsPlanCreating") : t("savings.savingsPlanSaveChanges")}</span>
                </button>
              </div>
            </form>
          </div>
