...
## Active Technologies
- TypeScript (project standard) + Next.js 16 App Router, React 19.2.4, Prisma ORM, TanStack Query, Formik, Tailwind CSS v4, Recharts
- Auth: custom JWT session cookie (`pf_session`) via `jose` + `bcryptjs` — no NextAuth/Better Auth
- i18n: react-i18next with a custom namespaced wrapper (`en-US`, `vi-VN` locales)
- Testing: Vitest (unit/integration) + Playwright (e2e)
- UI: Storybook-documented shared component library (`src/components/ui`) built on `class-variance-authority` and CSS custom-property design tokens
- Background jobs: optional Redis + BullMQ + node-cron for the savings month-end auto-transfer scheduler (no-ops gracefully when unconfigured)
- Storage: Prisma models on SQLite (local dev) — `User`, `UserSettings`, `Account`, `Category` (soft-delete via `deletedAt`, `isSystem`), `Transaction`, `SavingsPlan` (`status`, `isPrimary`), `AutoTransferRule`, `AutoTransferRun`, `UserMonthlyCap`, `CategoryMonthlyLimit`

## Recent Changes
- feat/core-component: Added core UI component library (Button, Avatar, Switch, Typography, Loading, Text, Checkbox) with Storybook stories and CSS-token styling; integrated into auth and settings views
- 009-group-transactions-date: Added ledger transaction date-grouping (Today/Yesterday/localized date headers)
- 008-delete-category: Added soft-delete category flow with confirmation dialog and reassignment to a system `Uncategorized` category
- 007-update-category-dialog: Added category edit dialog with case-insensitive unique-name validation
